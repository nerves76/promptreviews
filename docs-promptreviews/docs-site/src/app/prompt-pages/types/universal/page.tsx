...
const { Globe, Sparkles } = Icons

const fallbackDescription = 'The all-purpose review collection solution. One page that works for any customer, any time, anywhere. Perfect for businesses that want a simple, always-ready review collection tool.'
...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const article = await getArticleBySlug('prompt-pages/types/universal')
    if (!article) {
      return {
        title: 'The Universal Prompt Page - One Page for All Reviews | Prompt Reviews',
        description: fallbackDescription,
        alternates: {
          canonical: 'https://docs.promptreviews.app/prompt-pages/types/universal',
        },
      }
    }
    ...
    return {
      ...
      alternates: {
        canonical: article.metadata?.canonical_url ?? 'https://docs.promptreviews.app/prompt-pages/types/universal',
      },
    }
  } catch (error) {
    console.error('generateMetadata universal error:', error)
    return {
      title: 'The Universal Prompt Page | Prompt Reviews',
      description: fallbackDescription,
      alternates: {
        canonical: 'https://docs.promptreviews.app/prompt-pages/types/universal',
      },
...
export default async function UniversalPromptPagesPage() {
  let article = null

  try {
    article = await getArticleBySlug('prompt-pages/types/universal')
  } catch (error) {
    console.error('Error fetching universal article:', error)
  }
  ...
  return (
    <StandardOverviewLayout
      ...
      faqs={faqMetadata && faqMetadata.length ? faqMetadata : pageFAQs['prompt-pages/types/universal']}
      faqsTitle={faqsTitle}
