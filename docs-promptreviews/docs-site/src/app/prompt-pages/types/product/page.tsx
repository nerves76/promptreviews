...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const article = await getArticleBySlug('prompt-pages/types/product')
    if (!article) {
      return {
        title: 'Product Prompt Pages - Complete Guide | Prompt Reviews',
        description: fallbackDescription,
        alternates: {
          canonical: 'https://docs.promptreviews.app/prompt-pages/types/product',
        },
      }
    }
    ...
    return {
      ...
      alternates: {
        canonical: article.metadata?.canonical_url ?? 'https://docs.promptreviews.app/prompt-pages/types/product',
      },
    }
  } catch (error) {
    console.error('generateMetadata product error:', error)
    return {
      title: 'Product Prompt Pages | Prompt Reviews',
      description: fallbackDescription,
      alternates: {
        canonical: 'https://docs.promptreviews.app/prompt-pages/types/product',
      },
...
export default async function ProductPromptPagesPage() {
  let article = null

  try {
    article = await getArticleBySlug('prompt-pages/types/product')
  } catch (error) {
  ...
  return (
    <StandardOverviewLayout
      title={article.title || 'Product Prompt Pages'}
      ...
      faqs={faqMetadata && faqMetadata.length ? faqMetadata : pageFAQs['prompt-pages/types/product']}
      faqsTitle={faqsTitle}
