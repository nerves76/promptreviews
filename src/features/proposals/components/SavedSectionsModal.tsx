...
import { ProposalSectionTemplate } from '../types';
...
export function SavedSectionsModal({ isOpen, onClose, onImport }: SavedSectionsModalProps) {
  const [templates, setTemplates] = useState<ProposalSectionTemplate[]>([]);
  const [loading, setLoading] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<ProposalSectionTemplate | null>(null);
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      console.error('[SavedSectionsModal] Failed to fetch templates:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`/proposals/section-templates/${deleteTarget.id}`);
      setTemplates((prev) => prev.filter((t) => t.id !== deleteTarget.id));
    } catch (err) {
      console.error('[SavedSectionsModal] Failed to delete template:', err);
    } finally {
      setDeleting(false);
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Saved sections" size="lg">
        <Modal.Body>
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Save a section from any contract to build your library.
              </p>
            </div>
          ) : (
