...
export async function checkAccountLimits(supabase: SupabaseClient, userId: string, type: 'prompt_page' | 'contact') {
  // Fetch account
  ...
  if (limit !== Infinity && count >= limit) {
    return { allowed: false, reason: `Limit reached for your plan (${limit} ${type === 'prompt_page' ? 'prompt pages' : 'contacts'}).` };
  }
  return { allowed: true };
}

export async function isAccountBlocked(supabase: any, userId: string): Promise<boolean> {
  const { data: account } = await supabase.from('accounts').select('*').eq('id', userId).single();
