  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-90 animate-fadein">
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative animate-slideup border-2 border-blue-200">
          ...
          {emojiLabels.map((label, i) => {
            ...
            return (
              <button
                ...
                type="button"
              >
                <Icon className={`w-10 h-10 ${color} ${selected === i ? 'ring-2 ring-blue-400' : ''}`} />
        ...
        {selected !== null && !submitted && ['neutral','unsatisfied','angry'].includes(emojiLabels[selected].toLowerCase()) ? (
          <form
          ...
          >
            <div className="text-sm text-gray-700 mb-1 text-center">{feedbackMessage}</div>
            <textarea
          ...
          <div className="mt-6 text-center">
            <button
              ...
              onClick={() => {
                if (selected !== null && ['excellent','satisfied'].includes(emojiLabels[selected].toLowerCase())) {
