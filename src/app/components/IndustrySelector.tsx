...
export default function IndustrySelector({
  ...
  const filteredOptions = useMemo(() => {
    ...
    return INDUSTRY_OPTIONS.filter(opt => opt.type === industryType || opt.type === 'Both');
  }, [industryType]);

  // Convert to react-select format
  const selectOptions = filteredOptions.map(opt => ({ label: opt.label, value: opt.value }));
  ...
  const handleSelectChange = (selected: MultiValue<{ label: string; value: string }>) => {
    ...
    onChange(selectedValues, selectedValues.includes('Other') ? otherValue : undefined);
  };
