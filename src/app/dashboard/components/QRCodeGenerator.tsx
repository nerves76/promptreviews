...
export default function QRCodeGenerator({ url, clientName, frameSize = QR_FRAME_SIZES[0], onDownload }: QRCodeGeneratorProps) {
  ...
  const generateDesign = async () => {
    ...
    try {
      ...
      const headerToStarsGap = Math.floor(frameSize.height * 0.10);
      ...
      let logoY = frameSize.height - bottomPadding - logoDrawHeight - urlSpacing - Math.floor(frameSize.height * 0.021);
      ...
      if (urlY + bottomPadding > frameSize.height) {
        ...
        logoY = frameSize.height - bottomPadding - logoDrawHeight - urlSpacing - Math.floor(frameSize.height * 0.021);
        urlY = logoY + logoDrawHeight + urlSpacing;
      }
      ctx.font = `${Math.floor(frameSize.height * 0.021)}px Inter, Arial, sans-serif`;
      ctx.fillStyle = mainColor;
      ctx.textAlign = 'center';
      ctx.drawImage(svg, (frameSize.width - logoWidth) / 2, logoY, logoWidth, logoDrawHeight);
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      if (
        showStarPicker && starSwatchRef.current && !starSwatchRef.current.contains(e.target as Node)
      ) {
        setShowStarPicker(false);
      }
      if (
        showMainPicker && mainSwatchRef.current && !mainSwatchRef.current.contains(e.target as Node)
      ) {
      ...
      {previewUrl && (
        <div className="flex flex-row gap-8 justify-center items-center mb-4">
          <div className="relative flex items-center gap-2" ref={starSwatchRef}>
            <span className="text-sm font-medium text-gray-700">Star Color</span>
            <button
              type="button"
            ...
            />
            {showStarPicker && (
              <div className="absolute z-50 mt-2 left-1/2 -translate-x-1/2 bg-white p-2 rounded shadow-lg border" style={{ minWidth: 180 }}>
                <HexColorPicker color={starColor} onChange={setStarColor} />
                <div className="mt-1 text-xs text-gray-500 text-center">{starColor}</div>
              </div>
            )}
          </div>
          <div className="relative flex items-center gap-2" ref={mainSwatchRef}>
            <span className="text-sm font-medium text-gray-700">Main Color</span>
            <button
              type="button"
              className="w-8 h-8 rounded-full border-2 border-gray-300 shadow focus:outline-none focus:ring-2 focus:ring-slate-blue"
            ...
            {showMainPicker && (
              <div className="absolute z-50 mt-2 left-1/2 -translate-x-1/2 bg-white p-2 rounded shadow-lg border" style={{ minWidth: 180 }}>
                ...
                <div className="mt-1 text-xs text-gray-500 text-center">{mainColor}</div>
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 ml-4">
            ...
            />
            <label htmlFor="show-stars" className="text-sm text-gray-700 select-none cursor-pointer">Show stars</label>
          </div>
        </div>
      )}
      { !previewUrl && (
