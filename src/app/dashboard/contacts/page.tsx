...
import { FaDownload, FaUpload, FaInfoCircle, FaQuestionCircle, FaList, FaEye, FaUsers, FaTimesCircle, FaTimes } from 'react-icons/fa';
...
export default function UploadContactsPage() {
  ...
  const [selectedContactForPrompt, setSelectedContactForPrompt] = useState<any>(null);
  ...
  const expectedHeaders = [
    'first_name','last_name','email','phone','address_line1','address_line2','city','state','postal_code','country','business_name','notes','category'
  ];
  const isHeaderRowValid = preview.length > 0 && Array.isArray(preview[0]) && expectedHeaders.every((h, i) => (preview[0][i] || '').toLowerCase() === h);

  const handleUpload = async () => {
    if (!selectedFile) return;

    if (!isHeaderRowValid) {
      setError('The first row of your CSV must be the header row. Please remove any extra rows above your column headers.');
      return;
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${selectedContactIds.length} contact(s)? This cannot be undone.`)) return;
    try {
  ...
  let fieldKeys = contactFieldKeys.includes('category') ? contactFieldKeys : [...contactFieldKeys, 'category'];
  fieldKeys = fieldKeys.filter(k => k !== 'id' && k !== 'first_name' && k !== 'last_name' && k !== 'category');
  ...
  return (
    <PageCard icon={<FaUsers className="w-9 h-9 text-[#1A237E]" />}>
      <div className="w-full mx-auto relative" style={{maxWidth: 1000}}>
        ...
        {/* Contacts Table */}
        <div className="mb-16">
          <h2 className="text-2xl font-bold text-slate-blue mb-6">Your Contacts</h2>
          {anySelected && (
          ...
          {contactsLoading ? (
            ...
            <div className="text-gray-500">No contacts found. Upload your contacts to get started.</div>
          ) : (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {contacts.map((contact) => (
                    <tr key={contact.id}>
                      ...
                      <td className="px-3 py-2 text-sm text-gray-900">{contact.first_name} {contact.last_name}</td>
                      ...
                      <td className="px-3 py-2 text-sm text-gray-900">{contact.status}</td>
                      <td className="px-3 py-2 text-sm text-gray-900">{contact.category || ''}</td>
                      <td className="px-3 py-2 text-sm">
                        <button
        ...
        {/* Upload Modal */}
        <Dialog open={showUploadModal} onClose={() => setShowUploadModal(false)} className="fixed z-50 inset-0 overflow-y-auto">
        ...
        <Dialog open={showEditModal} onClose={() => setShowEditModal(false)} className="fixed z-50 inset-0 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4">
            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full mx-auto p-8 z-10">
              ...
              <Dialog.Title className="text-lg font-bold mb-4">Edit Contact</Dialog.Title>
              {editContact && (
                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input name="first_name" defaultValue={editContact.first_name || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Last Name</label>
                      <input name="last_name" defaultValue={editContact.last_name || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Email</label>
                      <input name="email" defaultValue={editContact.email || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Phone</label>
                      <input name="phone" defaultValue={editContact.phone || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Address Line 1</label>
                      <input name="address_line1" defaultValue={editContact.address_line1 || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Address Line 2</label>
                      <input name="address_line2" defaultValue={editContact.address_line2 || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">City</label>
                      <input name="city" defaultValue={editContact.city || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">State</label>
                      <input name="state" defaultValue={editContact.state || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Postal Code</label>
                      <input name="postal_code" defaultValue={editContact.postal_code || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Country</label>
                      <input name="country" defaultValue={editContact.country || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Business Name</label>
                      <input name="business_name" defaultValue={editContact.business_name || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Notes</label>
                      <input name="notes" defaultValue={editContact.notes || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700">Category</label>
                      <input name="category" defaultValue={editContact.category || ''} className="w-full border rounded px-2 py-1" />
                    </div>
                  </div>
                  {editError && <div className="mt-2 text-red-600 text-sm">{editError}</div>}
                  {editSuccess && <div className="mt-2 text-green-600 text-sm">{editSuccess}</div>}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
              ...
              <p className="mb-4 text-gray-600">
                You are about to delete {selectedContactIds.length} contact{selectedContactIds.length !== 1 ? 's' : ''}. This action cannot be undone.
              </p>
              <p className="mb-4 text-gray-600">
