...
const ProductPromptPageForm = forwardRef<any, ProductPromptPageFormProps>(
  ({ onSave, isLoading, initialData, showResetButton, businessReviewPlatforms = [], onGenerateReview, accountId }, ref) => {
    ...
    const [product_name, setProductName] = useState(initialData?.product_name ?? '');
    ...
    const [productPhotoUrl, setProductPhotoUrl] = useState(initialData?.product_photo || '');
    ...
    const handleProductPhotoUpload = async (supabase: any, accountId: string) => {
    ...
    }), [offerEnabled, offerTitle, offerBody, offerUrl, emojiSentimentEnabled, emojiSentimentQuestion, emojiFeedbackMessage, emojiThankYouMessage, emojiLabels, reviewPlatforms, fallingEnabled, fallingIcon, aiButtonEnabled, product_name, product_description, features_or_benefits, productPhotoFile, productPhotoUrl, onSave, supabase, accountId]);
        ...
        <form className="space-y-8" onSubmit={async e => {
          ...
          <div>
            ...
            <input
              ...
              onChange={e => { console.log('File input changed!'); handleProductPhotoChange(e); }}
            ...
            />
            {productPhotoError && (
              <div className="text-red-500 text-xs mt-1">{productPhotoError}</div>
            )}
