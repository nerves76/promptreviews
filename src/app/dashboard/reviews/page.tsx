...
function getSentimentIcon(sentiment: string) {
  ...
  if (idx !== -1) {
    ...
    return <Icon className={`w-7 h-7 ${color}`} title={EMOJI_SENTIMENT_LABELS[idx]} />;
  }
  return null;
}
...
export default function ReviewsPage() {
    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        const { data: existingReviews, error: fetchError } = await supabase
          .from("review_submissions")
          .select("id, prompt_page_id, first_name, last_name, reviewer_role, platform, review_content, created_at, status, emoji_sentiment_selection, verified, verified_at, platform_url")
  ...
  const filteredReviews = reviews.filter(r => {
    ...
    const emojiMatch = !emojiFilter || r.emoji_sentiment_selection === emojiFilter;
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      if (!(e.target as HTMLElement).closest('.review-popover') && !(e.target as HTMLElement).closest('.review-popover-btn')) {
  ...
  const handleMarkVerified = async (reviewId: string) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', reviewId);
      if (error) throw error;
      setReviews(prev => prev.map(r => r.id === reviewId ? { ...r, verified: true, verified_at: new Date().toISOString() } : r));
    } catch (err) {
  ...
  return (
    <PageCard>
      ...
      <div className="flex flex-wrap gap-4 mb-4 items-end justify-between">
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select className="border rounded px-2 py-1" value={platformFilter} onChange={e => setPlatformFilter(e.target.value)}>
              ...
              {[...Array.from(new Set(reviews.map(r => r.platform).filter(Boolean)))]?.map(p => (
          ...
          <div>
            ...
            <select className="border rounded px-2 py-1" value={verifiedFilter} onChange={e => setVerifiedFilter(e.target.value)}>
              <option value="">All</option>
        ...
        <button className="flex items-center gap-2 px-3 py-2 bg-slate-blue text-white rounded hover:bg-indigo-900 text-sm font-semibold ml-auto" onClick={handleExport}>
      ...
      <div className="space-y-4 mt-6">
        {filteredReviews.length === 0 ? (
          ...
          const { icon: PlatformIcon, label: platformLabel } = getPlatformIcon(review.platform);
          const isExpanded = expandedRows[review.id];
          return (
            <div key={review.id} className="bg-white rounded-lg border border-gray-200 shadow-sm">
              <button
                className="w-full flex items-center justify-between px-4 py-3 focus:outline-none"
                onClick={() => setExpandedRows(r => ({ ...r, [review.id]: !r[review.id] }))}
                aria-expanded={isExpanded}
                aria-controls={`review-details-${review.id}`}
              >
                <div className="flex items-center gap-3">
                  <PlatformIcon className="w-6 h-6 text-[#1A237E]" title={platformLabel} />
                  ...
                  {review.verified ? (
                    ...
                    <span className="ml-2 inline-block px-2 py-1 text-xs bg-gray-100 text-gray-500 rounded">Not verified</span>
                  )}
                </div>
                <span className="ml-4 text-gray-400">
                  {isExpanded ? <FaChevronDown className="w-4 h-4" /> : <FaChevronLeft className="w-4 h-4" />}
                </span>
              </button>
              {isExpanded && (
                <div id={`review-details-${review.id}`} className="px-4 pb-4 pt-2 border-t border-gray-100 animate-fade-in bg-gray-50 rounded-b-lg shadow-inner">
                  <div className="flex items-center gap-2 mb-2">
                    <FaRegComment className="w-5 h-5 text-slate-500" />
                    <span className="text-gray-800 text-sm" style={{whiteSpace: 'pre-line'}}>{review.review_content}</span>
                  </div>
                  {review.reviewer_role && (
                    <div className="text-xs text-gray-500 mb-1">Role: {review.reviewer_role}</div>
                  )}
