...
import { getDesignWithDefaults, hexToRgba, lightenHex, getRelativeTime } from '../../shared/utils';
...
const PhotoWidget: React.FC<{ data: WidgetData }> = ({ data }) => {
  ...
  const swiperConfig = React.useMemo(() => ({
    modules: [
      Navigation,
      Pagination,
      A11y,
  ...
  }), [design.autoAdvance, design.accentColor, design.slideshowSpeed]);

  // Handle Swiper initialization
  const handleSwiperInit = React.useCallback((swiper: any, isMobile: boolean = false) => {
    const updateNavigation = () => {
      if (!swiper || !swiper.params) return;

      if (isMobile) {
        if (prevRefMobile.current && nextRefMobile.current) {
          swiper.params.navigation.prevEl = prevRefMobile.current;
          swiper.params.navigation.nextEl = nextRefMobile.current;
          swiper.navigation.init();
          swiper.navigation.update();
        }
        if (paginationRefMobile.current) {
          swiper.params.pagination.el = paginationRefMobile.current;
          swiper.pagination.init();
          swiper.pagination.render();
          swiper.pagination.update();
        }
      } else {
        if (prevRefDesktop.current && nextRefDesktop.current) {
        ...
        if (paginationRefDesktop.current) {
          swiper.params.pagination.el = paginationRefDesktop.current;
          swiper.pagination.init();
          swiper.pagination.render();
          swiper.pagination.update();
        }
    ...
    if (isMobile) {
      setSwiperInstanceMobile(swiper);
    } else {
      setSwiperInstanceDesktop(swiper);
    }

    // Then update navigation in the next tick
          ...
          <div className="flex flex-col items-center w-full justify-center relative">
            ...
            <div className="hidden md:flex flex-row items-center justify-center w-full mt-2 gap-4">
              ...
              <div className="flex-1">
                <Swiper
                  {...swiperConfig}
                  onSwiper={(swiper) => handleSwiperInit(swiper, false)}
            ...
            <div className="md:hidden w-full">
              ...
              <Swiper
                ...
                onSwiper={(swiper) => handleSwiperInit(swiper, true)}
                modules={[
                  Navigation,
                  Pagination,
                  A11y,
