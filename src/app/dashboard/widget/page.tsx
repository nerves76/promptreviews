...
export default function WidgetPage() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('widget_reviews')
      .select('id, review_content, reviewer_name, reviewer_role, platform, created_at, order_index')
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select('id, review_content, reviewer_name, reviewer_role, platform, created_at')
    ...
    <div className="pb-12">
      ...
      <div className="mx-auto mb-12" style={{ maxWidth: design.showGrid ? 1000 : 800 }}>
        ...
        <section
          ...
          style={
            design.showGrid
              ? { background: 'none', color: design.textColor, border: 'none', boxShadow: 'none', padding: 0, minHeight: 320, margin: '0 auto' }
              : {
          ...
          {loading ? (
            ...
            <div className="relative w-full" style={{ minHeight: 220, padding: 0, overflow: 'hidden' }}>
              <Swiper
                ...
                spaceBetween={32}
                pagination={reviews.length > 1 ? { clickable: true, el: '.custom-swiper-pagination' } : false}
                loop={reviews.length > 3}
                autoplay={design.autoAdvance && reviews.length > 1 ? { delay: (design.slideshowSpeed ?? 4) * 1000, disableOnInteraction: false } : false}
              ...
              >
                {reviews.map((review) => (
                  <SwiperSlide key={review.id}>
                    ...
                    >
                      {design.showQuotes && (
                        <span className="absolute left-1/2 top-8 -translate-x-1/2 z-0 pointer-events-none" style={{ width: 96, height: 96, opacity: 0.5 }}>
                          <svg
                            width="96"
                            height="96"
                            viewBox="0 0 96 96"
                            ...
                            style={{ display: 'block' }}
                          >
                            <text x="20" y="76" fontSize="96" fill={lightenHex(design.accentColor, 0.7)} fontFamily="serif">“</text>
                          </svg>
                        </span>
                      )}
                      <p className="text-lg mb-2 md:mb-4 px-2 md:px-4 text-center" itemProp="reviewBody" style={{ lineHeight: design.lineSpacing, fontSize: design.quoteFontSize, color: design.textColor }}>
                        {review.review_content}
                      </p>
                      <div className="flex flex-col items-center gap-1 w-full mt-auto">
                        <span className="font-semibold text-indigo-700" itemProp="author" itemScope itemType="https://schema.org/Person" style={{ fontSize: design.attributionFontSize }}>
                          <span itemProp="name">{review.reviewer_name}</span>
                        </span>
                        <span className="text-xs text-gray-500" itemProp="author" itemScope itemType="https://schema.org/Person" style={{ fontSize: design.attributionFontSize * 0.85 }}>
                          <span itemProp="jobTitle">{review.reviewer_role}</span>
                        </span>
                        {design.showRelativeDate && review.created_at && review.platform && (
                          <span className="text-xs text-gray-400 mt-1">{getRelativeTime(review.created_at)} via {review.platform}</span>
                        )}
                      </div>
                    </article>
                  </SwiperSlide>
                ))}
                <div className="w-full flex justify-center mt-8 pointer-events-auto" style={{ minHeight: 40 }}>
                  <button
            ...
            <div className="flex flex-col items-center w-full">
              ...
              >
                ...
                <p className="text-lg mb-2 md:mb-4 px-2 md:px-8 text-center" itemProp="reviewBody" style={{ lineHeight: design.lineSpacing, fontSize: 20, color: design.textColor }}>
                  {reviews[current]?.review_content}
                </p>
                <div className="flex flex-col items-center gap-1 w-full mt-auto">
                  <span className="font-semibold text-indigo-700" itemProp="author" itemScope itemType="https://schema.org/Person" style={{ fontSize: design.attributionFontSize }}>
                    <span itemProp="name">{reviews[current]?.reviewer_name}</span>
                  </span>
                  <span className="text-xs text-gray-500" itemProp="author" itemScope itemType="https://schema.org/Person" style={{ fontSize: design.attributionFontSize * 0.85 }}>
                    <span itemProp="jobTitle">{reviews[current]?.reviewer_role}</span>
                  </span>
                  {design.showRelativeDate && reviews[current]?.created_at && reviews[current]?.platform && (
                    <span className="text-xs text-gray-400 mt-1">{getRelativeTime(reviews[current].created_at)} via {reviews[current].platform}</span>
                  )}
                </div>
              </article>
            </div>
        ...
        {!design.showGrid && reviews.length > 1 && (
          <div className="w-full flex items-center justify-center gap-0 mt-8 pointer-events-auto" style={{ minHeight: 40 }}>
            <button
