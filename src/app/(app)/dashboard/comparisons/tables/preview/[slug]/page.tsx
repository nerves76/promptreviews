...
import CompetitorSwapDropdown from "../../../components/CompetitorSwapDropdown";
...
export default function PreviewTablePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [displayedCompetitorIds, setDisplayedCompetitorIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Initialize displayed competitors from table config
        const initialCompetitorIds =
          data.table.table_type === "multi"
            ? data.table.competitor_ids || []
            : data.table.single_competitor_id
              ? [data.table.single_competitor_id]
              : [];
        setDisplayedCompetitorIds(initialCompetitorIds);
      } catch (err: any) {
  ...
  // Handler to swap one competitor for another
  const handleSwapCompetitor = (currentId: string, newId: string) => {
    setDisplayedCompetitorIds((prev) =>
      prev.map((id) => (id === currentId ? newId : id))
    );
  };

  // Get available competitors for swapping (not currently displayed)
  const getAvailableForSwap = (currentId: string) => {
    return allCompetitors.filter(
      (c) => c.id !== currentId && !displayedCompetitorIds.includes(c.id)
    );
  };
  ...
  // Use displayedCompetitorIds to get currently shown competitors (maintains order)
  const displayedCompetitors = displayedCompetitorIds
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-400">
      ...
      <div className="backdrop-blur-xl bg-white/10 border-b border-white/20 px-4 py-3 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          {/* Show reset button if competitors have been swapped */}
          {table && JSON.stringify(displayedCompetitorIds) !== JSON.stringify(
            table.table_type === "multi"
              ? table.competitor_ids || []
              : table.single_competitor_id ? [table.single_competitor_id] : []
          ) && (
            <button
              onClick={() => {
                setDisplayedCompetitorIds(
                  table.table_type === "multi"
                    ? table.competitor_ids || []
                    : table.single_competitor_id ? [table.single_competitor_id] : []
                );
              }}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-white/10 hover:bg-white/20 text-white/80 hover:text-white border border-white/20 transition-all"
            >
              <Icon name="FaRedo" size={10} />
              Reset to default
            </button>
          )}
          <span className="text-xs text-white/60">Resize browser to test responsiveness</span>
        </div>
      </div>
                    ...
                    <th className="px-4 py-6 text-center bg-white/10 border-x border-white/10">
                      <div className="relative group">
                        <div className="flex flex-col items-center gap-3 cursor-pointer">
                          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-500/30 transition-transform group-hover:scale-105">
                        ...
                        {/* Hover card tooltip */}
                        <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-64 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 delay-150 z-50 pointer-events-none group-hover:pointer-events-auto">
                          <div className="bg-white/10 backdrop-blur-xl border border-white/30 rounded-xl p-4 shadow-xl">
                            ...
                            <p className="text-sm text-white/90 leading-relaxed relative z-10">
                              Prompt Reviews is a review management platform built for small businesses and agencies. It uses AI to help customers write authentic reviews, with customizable prompt pages, keyword optimization, and multi-platform support. The platform includes review widgets, team collaboration, and detailed analytics to grow your online reputation.
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    {displayedCompetitors.map((comp) => {
                      ...
                      return (
                        <th key={comp.id} className="px-4 py-6 text-center">
                          <div className="relative group">
                            <div className="flex flex-col items-center gap-3 cursor-pointer">
                              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-300/50 to-purple-400/50 flex items-center justify-center overflow-hidden transition-transform group-hover:scale-105">
                                {comp.logo_url ? (
                                  <img
                                    src={comp.logo_url}
                                    alt={comp.name}
                              ...
                              {availableForSwap.length > 0 && (
                                <CompetitorSwapDropdown
                                  currentCompetitor={comp}
                                  availableCompetitors={availableForSwap}
                                  onSwap={(newId) => handleSwapCompetitor(comp.id, newId)}
                                  variant="preview"
                                />
                              )}
                            </div>
