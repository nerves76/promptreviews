...
export default function ResearchSubnav() {
  ...
  return (
    <div className="flex justify-center w-full mt-0 mb-0 z-20 px-4">
      <div className="flex overflow-x-auto scrollbar-hide bg-white/10 backdrop-blur-sm border border-white/30 rounded-full p-1 shadow-lg gap-0">
        {tabs.map((tab) => (
          <Link
            key={tab.href}
            href={tab.href}
            className={`px-6 py-1.5 font-semibold text-sm focus:outline-none transition-all duration-200 rounded-full flex items-center gap-2 flex-shrink-0 whitespace-nowrap
              ${tab.isActive
