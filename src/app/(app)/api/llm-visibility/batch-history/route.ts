/**
 * LLM Visibility Batch History API
 *
...
/**
 * GET /api/llm-visibility/batch-history
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (runsError) {
      console.error('❌ [LLMBatchHistory] Failed to fetch runs:', runsError);
    ...
    const runs = (batchRuns || []).map(run => ({
      runId: run.id,
      status: run.status,
      totalItems: run.total_questions,
      processedItems: run.processed_questions,
    ...
    console.error('❌ [LLMBatchHistory] Error:', error);
