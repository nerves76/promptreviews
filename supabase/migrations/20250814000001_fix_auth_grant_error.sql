-- Fix "Database error granting user" by removing problematic grant statements
-- and ensuring proper function permissions

-- Drop and recreate the trigger function without problematic GRANT
...
-- Note: We're NOT adding GRANT statements here as they can cause issues
-- The SECURITY DEFINER attribute is sufficient
...
-- Also update the manual account creation function
...
-- Ensure RLS bypass for the functions
ALTER FUNCTION handle_new_user_account() SECURITY DEFINER;
ALTER FUNCTION create_account_for_user(uuid) SECURITY DEFINER;

-- Add comment explaining the fix
COMMENT ON FUNCTION handle_new_user_account() IS 'Handles account creation for new users without problematic GRANT statements';