-- Add scheduled_for column to rank_batch_runs for scheduling runs at specific times
...
CREATE INDEX IF NOT EXISTS idx_rank_batch_runs_scheduled_for
ON rank_batch_runs(scheduled_for)
WHERE status IN ('pending', 'processing');

-- Add comment for documentation
COMMENT ON COLUMN rank_batch_runs.scheduled_for IS 'When the batch should start processing. NULL means start immediately.';
