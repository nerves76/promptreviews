-- Fix widget_reviews RLS policies to allow proper saving
-- This addresses the RLS policy violation when trying to save widget reviews
...
DROP POLICY IF EXISTS "Allow public read access" ON widget_reviews;

-- Re-enable RLS
ALTER TABLE widget_reviews ENABLE ROW LEVEL SECURITY;

-- Create simple, permissive policies that work with the current authentication setup
